...
description: Radar Charts with D3
date: 2020-06-15
tags: 
  - d3
...
Today I finished a [radar weather chart](https://chrisvaillancourt.github.io/d3-examples/radar-weather-chart/).

#### Thoughts

The radar chart is one of the more complex charts you can make. I have a new appreciation for the time it takes to build one and the time it takes to decipher visually.

I also used trigonometry for the first time since 10th grade so that was funny.

D3 tips:

- When choosing an area generator for a radar chart, use [`areaRadial()`](https://github.com/d3/d3-shape/blob/v1.3.7/README.md#areaRadial) instead of [`area()`](https://github.com/d3/d3-shape/blob/v1.3.7/README.md#area) because `area()` only accepts an x and y position (see `areaGenerator` below)
- When scaling a circle's radius, use [`scaleSqrt()`](https://github.com/d3/d3-scale#scaleSqrt) instead of [`scaleLinear()`](https://github.com/d3/d3-scale#scaleLinear) to keep scale circle size
