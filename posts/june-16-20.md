...
`clamp()` is a handy method to keep in mind when you absolutely need to prevent extrapolation beyond the defined range.

You can pad a scale's domain to force it to fit within a limited range but still have enough room to evenly space elements (see `startYScale` below).

#### Work examples

Initial sankey diagram (in all of its draft quality glory):
